...
const { generateEmail, generatePassword, generateName, generatePhoneNumber } = require('../utils/helper');
...
class LoginFashion extends Page {
    ...
    createAccount (name, lastname, email, pass) {
        ...
        cy.get(this.passInput)
        .type(pass, { force: true })
        cy.get(this.confirmPassIinput)
        .type(pass, { force: true })
    ...
    async fillBusinessInfo (companyName, phone) {
        cy.get(this.companyNameInput)
        .type(companyName, { force: true })
        cy.get(this.phoneInput)
        .type(phone, { force: true })
        cy.get(this.menuDropdownList)
        .eq(0)
        .click({ force: true })
        cy.contains(this.companyValue)
        .click({ force: true })
        cy.get(this.menuDropdownList)
        .eq(1)
        .click({ force: true })    
        cy.contains(this.companyStatusValue)
        .click({ force: true }) 
        cy.get(this.menuDropdownList)
        .eq(2)
        .click({ force: true })
        cy.contains(this.revenueValue)
        .click({ force: true })
        cy.get(this.branchDropdownBtn)
        .click({ force: true })
        cy.contains(this.branchValue)
        .click({ force: true })
        cy.get(this.menuDropdownList)
        .eq(3)
        .click({ force: true })
        cy.contains(this.phonePrefix)
        .click({ force: true })
        cy.wait(5000)
        cy.get(this.submitBtn)
        .click({ force: true })
